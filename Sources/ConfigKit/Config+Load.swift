...
public extension Config {
    ...
    ///
    /// - parameter file: The config file to load.  Should have the extension of .json or .plist
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
    /// - throws: An error of type Config.Error.fileNotFound if the method could not locate the file
    ...
    @discardableResult
    public func load(fromFile file: String) throws -> Config {
        // var path = p
        let path = NSString(string: NSString(string: file).expandingTildeInPath).standardizingPath
     ...
     /// Any objects with the same name as objects comming in from the load function will be replaced
     ///
     /// - parameter url: The config url to load.  Should have the extension of .json or .plist or have a correct mimeType set
     /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
     ///
     /// - throws: An error of type Config.Error.fileNotFound if the method could not locate the file
     /// An error of type Config.Error.unknownConfigType if it could not identify the file type
    ...
    ///
    /// - parameters:
    ///    - string: The string storing the configuration
    ///    - decoderType: Indicating what decoder to use
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
    ///
    /// - throws: A decoding error of the data is in an invalid format
    @discardableResult
    ...
    ///
    /// - parameters:
    ///    - data: The data storing the configuration
    ///    - decoderType: Indicating what decoder to use
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv(.load(fromFile: "global config.json").load(fromFile: "user config.json")
    ///
    ...
    /// Any objects with the same name as objects comming in from the load function will be replaced
    ///
    /// - parameter filter A way for you to filter which parameters to include or not
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
    @discardableResult
    ...
    ///    - seperator: An indicator for where to split the key and value (--paramName=value)
    ///    - filter: A way for you to filter which parameters to include or not
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
    @discardableResult
    ...
    /// Any objects with the same name as objects comming in from the load function will be replaced
    ///
    /// - parameter kv:The values to load
    ///
    /// - returns: Returns the same instance of the config object so that you can repeatedly call load functions within the same line eg.  config.loadFromEnv().load(fromFile: "global config.json").load(fromFile: "user config.json")
    @discardableResult
